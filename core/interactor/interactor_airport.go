...
func (i *AirportInteractor) ActivateAirport(ctx context.Context, id string) (err error) {
	...
	err = helpers.RunWithTx(ctx, i.service, log, logger.LogAirportActivateError,
		func(ctx context.Context, tx output.Tx) error {
			return i.service.ActivateAirportTx(ctx, tx, id)
		})
	if err != nil {
		return err
	}
...
func (i *AirportInteractor) DeactivateAirport(ctx context.Context, id string) (err error) {
	...
	err = helpers.RunWithTx(ctx, i.service, log, logger.LogAirportDeactivateError,
		func(ctx context.Context, tx output.Tx) error {
			return i.service.DeactivateAirportTx(ctx, tx, id)
		})
	if err != nil {
		return err
	}
