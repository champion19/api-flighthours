...
func TestNewMessageRepository_Success(t *testing.T) {
	...
	for i := 0; i < 9; i++ {
		mock.ExpectPrepare("SELECT|INSERT|DELETE")
	}
...
func TestNewMessageRepository_PrepareError(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
...
func TestMessageRepository_BeginTx_Success(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
...
func TestMessageRepository_BeginTx_Error(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatal(err)
	}
