...
func (r *repository) UpdateAirlineEmployeeStatus(ctx context.Context, tx output.Tx, id string, active bool) error {
	log.Debug(logger.LogAirlineEmployeeRepoStatusStart,
		"employee_id", id,
		"active", active)
	...
	if err != nil {
		log.Error(logger.LogAirlineEmployeeRepoStatusNotFound, "employee_id", id, "error", err)
		return domain.ErrAirlineEmployeeNotFound
	}

	if !hasAirline {
		log.Warn(logger.LogAirlineEmployeeRepoStatusNoInfo, "employee_id", id)
		return domain.ErrAirlineEmployeeNotFound
	}
	...
	if err != nil {
		log.Error(logger.LogAirlineEmployeeRepoStatusUpdateFail,
			"employee_id", id,
			"error", err)
		return err
	}

	log.Debug(logger.LogAirlineEmployeeRepoStatusOK,
		"employee_id", id,
		"active", active)
