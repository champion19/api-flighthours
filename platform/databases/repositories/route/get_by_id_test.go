...
func TestGetRouteByID_Success(t *testing.T) {
	...
	cols := []string{"id", "origin_airport_id", "origin_iata_code", "origin_airport_name",
	...
	prep.ExpectQuery().WithArgs("r1").WillReturnRows(
		sqlmock.NewRows(cols).AddRow("r1", "ap1", "BOG", "El Dorado", "ap2", "JFK", "Kennedy", "International", sql.NullString{String: "5h", Valid: true}, "BOG-JFK"),
	)
